...
from scripts.ghost_trader import process_user_commands, execute_emergency_liquidation
...
async def test_pause_resume_state(memory_state, mock_notifier):
    ...
    assert scripts.ghost_trader.SYSTEM_PAUSED is True
    
    memory_state.write_command("/resume")
    await process_user_commands(memory_state, pairs=[], notifier=mock_notifier)
    assert scripts.ghost_trader.SYSTEM_PAUSED is False
...
async def test_execute_emergency_liquidation(memory_state, mock_notifier):
    ...
    with patch("scripts.ghost_trader.fetch_live_klines", new_callable=AsyncMock) as mock_fetch:
        ...
        await execute_emergency_liquidation(memory_state, pairs=[], notifier=mock_notifier, target=None)
