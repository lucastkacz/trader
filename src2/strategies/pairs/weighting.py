...
def calculate_beta_neutral_weights(
    ...
    # Apply directional leverage
    is_long = positions == 1.0
    is_short = positions == -1.0
    
    weights.loc[is_long, asset_a] *= long_leverage
    weights.loc[is_long, asset_b] *= long_leverage
    
    weights.loc[is_short, asset_a] *= short_leverage
    weights.loc[is_short, asset_b] *= short_leverage
