...
* **Zero Default Values in Function Signatures:** No function in the codebase is permitted to declare default values for parameters that originate from configuration (timeframe, exchange, limit, volume thresholds, file paths, etc.). All such values must be explicitly passed from the YAML config at the call site. Default values are only acceptable for genuinely optional behavior flags (e.g., `verbose: bool = False`) or `None` sentinel values for optional parameters. This guarantees that every operational parameter is traceable back to a single YAML source of truth.
* **Exchange Agnosticism:** The codebase must never hardcode exchange-specific logic (e.g., `if exchange == "bybit"`). The exchange identifier is passed as a raw CCXT exchange ID (e.g., `"bybit"`, `"binanceusdm"`, `"kucoin"`) directly from the pipeline YAML. The system must never attempt to infer, map, or cast exchange identifiers internally.
