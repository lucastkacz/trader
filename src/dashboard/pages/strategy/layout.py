...
def render_strategy_page():
    ...
    with st.expander("⚙️ Execution Parameters (Z-Score & Portfolio)", expanded=False):
        ...
        with c1:
            coint_window_input = st.number_input(
                ...
                help="Window for calculating dynamic rolling spread and P-Value."
            )
        with c2:
            z_window = st.number_input("Z-Score Moving Average", min_value=10, value=30, help="Smoothing for entry signals.")
        with c3:
    ...
    # Module 2
    spread, p_values, rolling_beta = render_spread_analysis(df_pair, asset_a, asset_b, window=coint_window_input)
    st.divider()
    
    # Module 3
    signals = render_signal_gen(spread, p_values, z_window=z_window, z_entry=z_entry, z_exit=z_exit)
    if signals is None:
        return
    st.divider()
